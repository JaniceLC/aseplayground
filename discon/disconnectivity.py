...
class _MakeTree(object):
    ...
    def make_tree(self):
        ...
        tslist = [ts for ts in self.transition_states if ts[0] != ts[1]]
        # remove duplicate entries and sort
        #print(len(tslist))
        #print(tslist)
        #tslist = list(set(tslist))
        tslist.sort(key=lambda ts: -ts[2])
    ...
    def _do_next_level(self, ilevel, previous_trees):
        ...
        while len(tslist) > 0:
            ...
            new_minima = self._add_edge(ts[0], ts[1])
            for m in new_minima:
    ...
    def tree_get_colors(self, tree):
        ...
        try:
            ...
            if tree.is_leaf():
                ...
                colors_list = [self.tree_get_colors(
                    subtree) for subtree in tree.get_branches()]
    ...
    def tree_get_value(self, tree):
        ...
        try:
            ...
            if tree.is_leaf():
                ...
                values = [self.tree_get_value(subtree)
...
class DisconnectivityGraph(object):
    ...
    def __init__(self, graph, minima=None, nlevels=20, Emax=None,
                 ...
                 order_by_value=None, csv_map=None, energy_csv=None):
        self.graph = graph
    ...
    def _getEnergy(self, node):
        ...
        try: 
            E = self.energy_csv[self.energy_csv['m1'] == node]['E1'].values[0]
        except:
            E = self.energy_csv[self.energy_csv['m2'] == node]['E2'][0]
        return E
    def _getEnergyedge(self, edge):
        """ get the energy of a node """
        try:
            E = self.energy_csv[self.energy_csv['m1'] == node]['E1']
        except:
            E = self.energy_csv[self.energy_csv['m2'] == node]['E2']
        return E
        ...
        if len(rmlist) > 0:
            if self.gmin0 is not None:
                if self.gmin0 in rmlist:
                    print("global minimum has", graph.degree(
                        self.gmin0), "edges, not showing in graph")
            print("removing", len(rmlist),
                  "minima from graph with fewer than", nmin, "edges")
            for n in rmlist:
                graph.remove_node(n)
        return graph
    ...
    def _remove_high_energy_minima(self, graph, emax):
        ...
        if len(rmlist) > 0:
            print("removing %d nodes with energy higher than" %
                  len(rmlist), emax)
        for m in rmlist:
            graph.remove_node(m)
        return graph
    ...
    def _remove_high_energy_transitions(self, graph, emax):
        ...
        if len(rmlist) > 0:
            print("removing %d edges with energy higher than" %
                  len(rmlist), emax)
        for edge in rmlist:
